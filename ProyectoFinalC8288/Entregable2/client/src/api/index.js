...
export const hahaReact = (id) => API.patch(`/posts/${id}/hahaReact`);
...
// commentPost es una función que se exporta para crear comentarios en posts
// Recibe dos parámetros:
// id: el ID del post donde se añadirá el comentario 
// comment: objeto con los datos del comentario
export const commentPost = (id, comment) =>
    // API.post hace una petición POST al servidor
    // El primer argumento usa template literals (``) para construir la URL
    // /posts/${id}/comment se convierte en algo como /posts/123/comment
    // El segundo argumento comment es el objeto con el contenido del comentario
    // Por ejemplo: { content: "Buen post!", name: "Usuario" }
    // También incluirá automáticamente el token JWT en los headers
    API.post(`/posts/${id}/comment`, comment);
    // Esta función retorna una promesa que será manejada por el action creator
    
 // En commentPost, una petición completa se vería así:
 // Petición POST a http://localhost:5000/posts/123/comment
 // Headers: {
 ...
 // estas funciones son parte del módulo api/index.js que centraliza
 // todas las llamadas HTTP al backend usando Axios
 // Se usan en los action creators correspondientes para realizar
 // las operaciones y luego actualizar el estado de Redux